...
def dijkstras_shortest_path(initial_position, destination, graph, adj):
    ...
    pri_q = []
    heappush(pri_q, (0, initial_position))
    visited = {}
    visited[initial_position] = None
    cum_cost = {}
    cum_cost[initial_position] = 0
    path = []

    while len(pri_q) != 0:
        current = heappop(pri_q)
    ...
    while True:
        ...
        if current is initial_position:
            path.append(current)
            return path
...
def dijkstras_shortest_path_to_all(initial_position, graph, adj):
    ...
    heappush(heap_queue, (0, initial_position))
    ...
    while len(heap_queue) != 0:
        ...
        adj_list = adj(graph, current[1])
    
        for x in adj_list:

            new_cost = cost_dict[current[1]] + x[1] 
            ...
            if x[0] not in cost_dict or new_cost < cost_dict[x[0]]:
                ...
                heappush(heap_queue, (new_cost, x[0]))
...
def navigation_edges(level, cell):
    ...
    """
    # TTTTTTTTTTTTTTTTTTTTTTTTTTTTTEEEEEEEEEEEEEEEEEMMMMMMMMMMMMMPPPPPPPP
    adj_list = []
