...
List allAnagrams(string input)
{
    ...
    while(!recurStack.empty())
    {
        ...
        if(top.in_str.length() == 0)
        {
            L.insert(top.fixed_str);
        }
        else
        {
            temp = top;
            for(int i = 0; (unsigned)i <= top.in_str.length(); i++)
            {
                top = temp;
                top.fixed_str = top.fixed_str + top.in_str[i];
                top.in_str.erase(i, 1);
                recurStack.push(top);
            }
        }
    }
...
List stretch(string input, int k)
{
    ...
    while(!recurStack.empty())
    {
        ...
        {
            char c = top.in_str.front();
            top.in_str.erase(0, 1);
            for(int i = 0; i < k; i++)
            {
                top.fixed_str = top.fixed_str + c;
                recurStack.push(top);
            }
        }
    }
