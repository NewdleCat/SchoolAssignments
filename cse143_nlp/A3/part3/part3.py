...
def decode(input_length, tagset, score):
    ...
    b = [[0 for x in range(M)] for y in range(K)] 

    def getMax(MM, KK):
        temp = []
        for k in range(K):
            temp.append(score(tagset[KK], tagset[k], MM) + v[k][MM-1])
        return max(temp), temp.index(max(temp))

    for k in range(K):
        v[k][1] = score(tagset[k], '<START>', 1)

    for m in range(2, M-1):
        for k in range(0, K):
            v[k][m], b[k][m] = getMax(m, k)

    y = [0]*M
    temp = []
    for k in range(K):
        temp.append(score('<STOP>', tagset[k], M-1) + v[k][M-2])

    y[M-1] = temp.index(max(temp))
